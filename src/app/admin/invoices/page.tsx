...
import { FiPlus, FiTrash2, FiPrinter, FiList } from "react-icons/fi";
...
export default function InvoicesPage() {
  ...
  const handleSave = async () => {
    ...
    const row = {
      ...
      event_type: null, event_date: invoice.date, items: invoice.items,
    ...
    <div>
      <header className="bg-white shadow-sm px-4 sm:px-6 py-4 flex items-center justify-between sticky top-0 z-30">
        ...
        <div className="flex gap-2">
          {mode === "list" && <button onClick={newInvoice} className="bg-primary text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2"><FiPlus className="w-4 h-4" /><span className="hidden sm:inline">New Invoice</span></button>}
        ...
        {mode === "preview" && (
          <div>
            <div ref={printRef}>
              ...
              <div style={{marginBottom:25}}><div style={{fontSize:10,fontWeight:"bold",color:"#999",textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Bill To</div><div style={{fontWeight:"bold",color:"#1a1a2e",fontSize:15}}>{invoice.customer_name || "Customer Name"}</div>{invoice.customer_phone && <div style={{fontSize:12,color:"#666"}}>{invoice.customer_phone}</div>}{invoice.customer_email && <div style={{fontSize:12,color:"#666"}}>{invoice.customer_email}</div>}</div>
