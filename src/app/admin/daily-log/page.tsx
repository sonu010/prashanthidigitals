...
export default function DailyLogPage() {
  ...
  const mkForm = () => ({ date: new Date().toISOString().split("T")[0], type: "event" as LogEntry["type"], title: "", description: "", amount: undefined as number | undefined, customer_name: "", customer_phone: "" });
  const [form, setForm] = useState(mkForm());

  const fetchLogs = useCallback(async () => {
    const { data } = await supabase.from("daily_logs").select("*").order("date", { ascending: false }).order("created_at", { ascending: false });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const row = { date: form.date, type: form.type, title: form.title, description: form.description, amount: form.amount || null, customer_name: form.customer_name || null, customer_phone: form.customer_phone || null };
    if (editingId) {
  ...
  const handleEdit = (log: LogEntry) => {
    setForm({ date: log.date, type: log.type, title: log.title, description: log.description, amount: log.amount, customer_name: log.customer_name || "", customer_phone: log.customer_phone || "" });
    setEditingId(log.id);
    setShowForm(true);
  };
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
        ...
        {showForm && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-6">
            <h3 className="font-bold text-accent mb-4">{editingId ? "Edit Entry" : "New Log Entry"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Customer</label><input type="text" value={form.customer_name} onChange={(e) => setForm({ ...form, customer_name: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary outline-none" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Phone</label><input type="tel" value={form.customer_phone} onChange={(e) => setForm({ ...form, customer_phone: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary outline-none" /></div>
              </div>
              <div className="flex gap-3"><button type="submit" disabled={saving} className="bg-primary text-white px-6 py-2.5 rounded-lg font-semibold text-sm disabled:opacity-50">{saving ? "Saving..." : editingId ? "Update" : "Save"}</button><button type="button" onClick={() => { setShowForm(false); setEditingId(null); }} className="px-6 py-2.5 text-sm text-gray-600">Cancel</button></div>
            </form>
          </div>
        )}
        {dates.length === 0 ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {log.amount !== undefined && log.amount !== null && log.amount > 0 && <span className={"text-sm font-bold " + (log.type === "expense" ? "text-red-600" : "text-green-600")}>{log.type === "expense" ? "-" : "+"}&#8377;{log.amount.toLocaleString()}</span>}
                      </div>
                      ...
                      {log.customer_name && <p className="text-xs text-gray-400 mt-1">Customer: {log.customer_name}{log.customer_phone && " - " + log.customer_phone}</p>}
                    </div>
